          ...
          <nav className={`${mobileMenuOpen ? "block" : "hidden"} md:block mt-4 md:mt-0`}>
            <ul className="flex flex-col md:flex-row w-full max-w-maxContent items-center justify-between px-4 py-2 gap-y-4 md:gap-y-0 md:gap-x-14">
              {NavbarLinks.map(({ title, path }, index) => (
                <li
                  key={index}
                  onMouseEnter={title === "Catalog" ? handleMouseEnter : undefined}
                  onMouseLeave={title === "Catalog" ? handleMouseLeave : undefined}
                ...
                >
                  {title === "Catalog" ? (
                    ...
                    >
                      ...
                      <BsChevronDown />
                      {dropdownOpen && (
                        <div className="absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 transition-all duration-150 group-hover:translate-y-[1.65em] lg:w-[300px]">
